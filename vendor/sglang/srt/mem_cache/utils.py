...
try:
    ...
    class _MissingEnvVar:
        def get(self):
            return None

    class _EnvFallback:
        SGLANG_MOONCAKE_CUSTOM_MEM_POOL = _MissingEnvVar()
...
try:
    ...
    def _missing_triton_op(*args, **kwargs):
        raise RuntimeError("Triton MLA buffer ops are unavailable in CPU vendor mode.")

    get_mla_kv_buffer_kernel = _missing_triton_op
    get_mla_kv_buffer_triton = _missing_triton_op
    set_mla_kv_buffer_fp8_quant_kernel = _missing_triton_op
    set_mla_kv_buffer_kernel = _missing_triton_op
    set_mla_kv_buffer_triton = _missing_triton_op
    set_mla_kv_buffer_triton_fp8_quant = _missing_triton_op
    set_mla_kv_scale_buffer_kernel = _missing_triton_op
    set_mla_kv_scale_buffer_triton = _missing_triton_op
